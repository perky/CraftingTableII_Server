...
public class ContainerClevercraft extends Container {
	...
	public ContainerClevercraft(InventoryPlayer inventoryplayer, World world)
	{
		...
		thePlayer.currentCraftingInventory = this;
	...
	public void populateSlotsWithRecipes()
	{
		...
		for(int i = 0; i < recipeList.size(); i++) {
			...
			for(int j = 0; j < 36; j++){
				if(thePlayer.inventory.mainInventory[j] != null)
					mainInventory[j] = thePlayer.inventory.mainInventory[j].copy();
			}
			...
			for(int i1 = 0; i1 < recipeIngredients.length; i1++) {
				...
				int slotindex = getFirstInventoryPlayerSlotWithItemStack(mainInventory, itemstack);
				if(slotindex != -1) {
					mainInventory[slotindex].stackSize -= itemstack.stackSize;
					if(mainInventory[slotindex].stackSize <= 0)
						mainInventory[slotindex] = null;
				} else {
	...
	private int getFirstInventoryPlayerSlotWithItemStack(ItemStack[] inventory, ItemStack itemstack)
	{
		for(int i = 0; i < inventory.length; i++) {
			ItemStack itemstack1 = inventory[i];
			if(itemstack1 != null
    ...
    */
	...
	public void onCraftMatrixChanged(ItemStack recipeOutputStack)
	{
		InventoryPlayer inventoryPlayer = thePlayer.inventory;
		// Call custom hooks.
